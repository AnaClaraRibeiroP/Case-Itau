...
output_file = r'c:\Users\aclarari\Desktop\NOVO CASE\DADOS\base_auxiliar_cast.csv'
...
for index, row in df.iterrows():
    # Verificar se a coluna 'cast' não está vazia
    if pd.notna(row['cast']):
        # Dividir os nomes da coluna 'cast' por vírgula
        names = row['cast'].split(', ')
        # Para cada nome, adicionar uma entrada com show_id e o nome para aux_data
        for name in names:
            aux_data.append({'show_id': row['show_id'], 'cast': name})
...
print(f'Arquivo base_auxiliar_cast.csv salvo com sucesso em {output_file}')
